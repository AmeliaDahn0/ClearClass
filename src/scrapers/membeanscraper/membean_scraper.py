...
class DataCollector:
    ...
    def load_or_create_today_file(self):
        """Load today's file if it exists, or create new data structure if it doesn't"""
        os.makedirs('data', exist_ok=True)
        today = datetime.now().strftime('%Y-%m-%d')
        self.filename = f'data/membean_data_{today}.json'
        
        # Try to load existing file from today
        try:
            if os.path.exists(self.filename):
                with open(self.filename, 'r') as f:
                    self.data = json.load(f)
                print(f"Loaded existing data file: {self.filename}")
            else:
                ...
                print(f"Creating new data file for today: {self.filename}")
        except json.JSONDecodeError:
            print(f"Error reading {self.filename}, creating new data structure")
            self.data = {
                'timestamp': datetime.now().isoformat(),
                'url': None,
                'students': {}
    ...
    def add_tab_data(self, tab_name: str, tab_data: Dict):
        ...
        # If this is Reports tab data and it contains a URL, store it at the top level
        if tab_name == "Reports" and "url" in tab_data:
            self.data['url'] = tab_data.pop('url')
            # The remaining student data is in the 'students' field
            tab_data = tab_data.get('students', {})
        
        # Add the tab data for each student
        ...
        # Update timestamp each time we add data
        self.data['timestamp'] = datetime.now().isoformat()
        
        # Save after each tab update to ensure we don't lose data
        self.save_to_file()
